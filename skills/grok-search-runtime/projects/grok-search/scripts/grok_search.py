#!/usr/bin/python3
"""Grok search via xAI Responses API with web_search tool.

- Primary: grok-4.20-0309-reasoning (with built-in web_search)
- Fallback: grok-4-1-fast-reasoning
- Uses /v1/responses endpoint with tools=[{"type":"web_search"}]
"""
...
def request_responses(
    ...
    """Call xAI Responses API with built-in web_search tool."""
    payload = {
        "model": model,
        "input": messages,
...
def call_with_validation(
    ...
    try:
        resp_json = request_responses(base_url, api_key, model, messages, timeout)
...
def parse_args():
    ...
    default_model = get_config(["GROK_MODEL"], "model", "grok-4.20-0309-reasoning")
    default_base_url = get_config(
        ["GROK_BASE_URL", "GAPI_CUSTOM_BASE_URL"],
        "base-url",
        "https://api.x.ai",
    )
    default_api_key = get_config(["GROK_API_KEY", "GAPI_CUSTOM_API_KEY", "XAI_API_KEY"], "api-key")
    try:
    ...
    p.add_argument("--fallback-model", default=os.getenv("GROK_FALLBACK_MODEL", "grok-4-1-fast-reasoning"))

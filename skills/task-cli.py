...
def cmd_notification(args):
    ...
    print(f"  渠道: {', '.join(str(c) for c in data['channels']) if data['channels'] else '未配置'}")
...
def cmd_notify(args):
    """发送 Telegram 通知"""
    import subprocess
    script = pathlib.Path(__file__).resolve().parent / "notify.py"
    if not script.exists():
        print("❌ notify.py 不存在")
        return
    result = subprocess.run(
        ["python3", str(script), args.message],
...
def cmd_skill_list(args):
    ...
    r = requests.get(f"{BASE_URL}/api/tools/skills", headers=headers, params=params)
    data = r.json()
...
def cmd_skill_get(args):
    """获取指定技能的详细内容"""
    headers = _headers(args.key)
    r = requests.get(f"{BASE_URL}/api/tools/skills/{args.name}", headers=headers)
    if r.status_code == 200:
        print(r.text)
    else:
...
def main():
    ...
    # --- notify ---
    p = subparsers.add_parser("notify", help="发送 Telegram 通知")
    ...
    # --- skill ---
    skill_p = subparsers.add_parser("skill", help="技能发现与查看")
    skill_sub = skill_p.add_subparsers(dest="skill_cmd")

    p = skill_sub.add_parser("list", help="列出所有可用技能")
    p.add_argument("--search", help="按关键词搜索")
    p.set_defaults(func=cmd_skill_list)

    p = skill_sub.add_parser("get", help="查看指定技能详情")
    p.add_argument("name", help="技能名称")
    p.set_defaults(func=cmd_skill_get)

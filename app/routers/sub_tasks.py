...
async def create_sub_task(
    ...
    if req.assigned_agent:
        agent_trigger.on_subtask_assigned(sub_task.id)
    return sub_task
...
async def submit_sub_task(
    ...
    try:
        result = sub_task_service.submit_sub_task(db, sub_task_id)
        agent_trigger.on_subtask_submitted(sub_task_id)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def complete_sub_task(
    ...
    """审查通过：review → done"""
    try:
        result = sub_task_service.complete_sub_task(db, sub_task_id)
        agent_trigger.on_subtask_done(sub_task_id)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def rework_sub_task(
    ...
    """驳回返工：review → rework"""
    try:
        result = sub_task_service.rework_sub_task(db, sub_task_id, req.rework_agent)
        agent_trigger.on_subtask_rework(sub_task_id)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def block_sub_task(
    ...
    try:
        result = sub_task_service.block_sub_task(db, sub_task_id)
        agent_trigger.on_subtask_blocked(sub_task_id)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
